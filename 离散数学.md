...
**例$_2$**：在 $\mathbb{R}$ 中定义二元运算 $*$：$\forall a,b\in\mathbb{R}$，$a*b=a+b+ab$。证明：$<\mathbb{R}, *>$ 构成独异点。

**证**：
1. **封闭性**  
   $\because \forall a,b\in\mathbb{R}$，$a\circb=a+b+ab\in\mathbb{R}$  
   $\therefore \circ$ 封闭。
...
3. **单位元存在性**  
   对 $0\in\mathbb{R}$，$\forall x\in\mathbb{R}$，有 $0\circ x = 0+x+0x = x$，$\therefore 0$ 为单位元

**综上**，$<\mathbb{R}, \circ>$ 为独异点。
