...
2. 半群 $V=\langle  S, \circ \rangle$ 中 $\circ$ 运算含有幺元，称 $V$ 为**含幺半群 (独异点)**, 记作 $\langle  S, \circ, e \rangle$；
...
**例$_1$**：$\langle N, +  \rangle$ 是**含幺半群**，因为：① 是可结合的；② 含有单位元 $0$ 
    $\langle N, +  \rangle$ 不是**群**，因为不满足 $\forall x \in S$，有 $x^{-1} \in S$，如 $5$ 的逆元 $-5\notin \mathbb{N}$
