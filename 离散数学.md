...
设 $G_1=\langle V_1,E_1\rangle$，$G_2=\langle V_2,E_2\rangle$ 为两个无向图， 若存在双射函数 $f:V_1\to V_2$，使得 $\forall v_i,v_j\in V_1$， $(v_i,v_j)\in E_1$ 当且仅当 $(f(v_i),f(v_j))\in E_2$； 
$\langle v_i,v_j\rangle\in E_1$ 当且仅当 $\langle f(v_i),f(v_j)\rangle\in E_2$， 并且 $(v_i,v_j)$ 与 $(f(v_i),f(v_j))$、$\langle v_i,v_j\rangle$ 与 $\langle f(v_i),f(v_j)\rangle$ 的重数相同， 则称 $G_1$ 与 $G_2$ **同构**，记作 $G_1\cong G_2$。
