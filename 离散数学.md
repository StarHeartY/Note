...
**例$_1$**：$<N, + >$ 是**含幺半群**，因为：① 是可结合的；② 含有单位元 $0$ 
    $<N, + >$ 不是**群**，因为不满足 $\forall x \in S$，有 $x^{-1} \in S$，如 $5$ 的逆元 $-5\notin \mathbb{N}$


**例$_2$**：在 $\mathbb{R}$ 中定义二元运算 $\circ$：$\forall a,b\in\mathbb{R}$，$a\circ b=a+b+ab$。证明：$<\mathbb{R}, \circ>$ 构成独异点。
...
**综上**，$<\mathbb{R}, \circ>$ 构成独异点。

### 3. 子群

若 $<G, \circ>$ 和 $<H, \circ>$ 都为群，$H\subseteq G$，$H\neq\emptyset$，则称 $H$ 为 $G$ 的**子群**，记作 $H\leq G$。

其中，最小的子群是只含单位元的子群 $<\{e\}, \circ>$，最大的子群是群本身 $<G, \circ>$
	  两者称为 $<G, \circ>$ 的**平凡子群**。

#### 子群的判定条件

1. $\begin{cases} \boldsymbol{①}\ \forall a,b\in H，有\ ab\in H\\ \boldsymbol{②}\ \forall a\in H，有\ a^{-1}\in H \end{cases}$
2. $\forall a,b\in H$，有 $ab^{-1}\in H$
3. $H$ 是有限集，$H$ 是 $G$ 子群 $\iff$ $\forall a,b\in H$，有 $ab\in H$
4. 