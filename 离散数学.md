...
- 若 $G$ 是 $n$ 阶循环群，则 $G$ 含有 $\boldsymbol{\phi(n)}$ 个生成元。 对于任何小于 $n$ 且与 $n$ 互素的自然数 $r$，$a^r$ 是 $G$ 的生成元。 
> 注：$\phi(n)$ 是欧拉函数，表示小于等于 $n$ 且与 $n$ 互素的正整数的个数。
...
3. **幂等律** $a \wedge a = a$ $a \vee a = a$ 
4. **分配律（相互分配，核心）** $a \wedge (b \vee c) = (a \wedge b) \vee (a \wedge c)$ $a \vee (b \wedge c) = (a \vee b) \wedge (a \vee c)$ 
5. **吸收律** $a \wedge (a \vee b) = a$ $a \vee (a \wedge b) = a$ 
6. **同一律** $a \wedge 1 = a$ $a \vee 0 = a$ 
7. **零律** $a \wedge 0 = 0$ $a \vee 1 = 1$ 
8. **互补律（否定律）** $a \wedge a' = 0$ $a \vee a' = 1$
9. **双重否定律** $\forall a \in B,\; (a')' = a$ 
10. **德摩根律** $\forall a,b \in B,\; (a\wedge b)' = a'\vee b',\; (a\vee b)' = a'\wedge b'$
