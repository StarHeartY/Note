...
在大量重复试验中，样本平均数会趋近于理论期望值。换句话说，当试验次数足够多时，实验结果的平均值会接近预期的长期平均值。

**中心极限定理**：设 $n$ 个随机变量 $X_1, X_2, X_3, \cdots, X_n$ 相互独立且服从同一种分布规律（独立同分布），期望值为 $\mu$，方差为 $D$，则有

$$
\frac{\sum_{i=1}^n X_i - n\mu}{\sqrt{nD}} \sim N(0,1) \quad \text{或} \quad \sum_{i=1}^n X_i \sim N(n\mu, nD)
$$

$$
\frac{\overline{X} - \mu}{\sqrt{D/n}} \sim N(0,1) \quad \text{或} \quad \overline{X} \sim N\left(\mu, \frac{D}{n}\right)
$$